...
- **Background (后台)**: Service Worker 负责调度统筹管理后台长任务流的 `TaskManager`、AI 服务调用（含并发控制、Token 跟踪与 `<think>` 标签清洗）以及统一规范化的 `StorageManager`（支持以透明化外观模式结合 `lz-string` 压缩将数据快速送入特定的历史/设置/日志库）。
- **Content Scripts (内容脚本)**: 执行带有 `MutationObserver` 网络监听器的 DOM 滚动交互模拟任务（点击"查看回复"并等待）和深层数据提取。
...
3. **进度监控**: 在弹窗中实时查看提取进度。
4. **洞察分析**: 提取完成后，点击"分析评论"生成深度报告。
5. **查看详情**: 进入"历史记录"查看详细评论数据和 AI 分析结果。
