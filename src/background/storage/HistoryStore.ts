...
export class HistoryStore {
    ...
    async getHistoryItem(id: string): Promise<HistoryItem | undefined> {
        try {
            ...
            const decompressed = compressedComments ? LZString.decompressFromUTF16(compressedComments) : null;
    ...
    private async getOrBuildSortedIndex(): Promise<HistorySortedIndex> {
        ...
        try {
            ...
            const storedIndex = result[STORAGE.HISTORY_SORTED_INDEX_KEY] as HistorySortedIndex | undefined;
    ...
    private async rebuildSortedIndex(): Promise<HistorySortedIndex> {
        try {
            ...
            for (const id of ids) {
                const baseKey = `${STORAGE.HISTORY_KEY}_${id}`;
                const result = await chrome.storage.local.get(baseKey);
                const item = result[baseKey] as { id: string; extractedAt: number; url: string; title: string; platform: string; } | undefined;
                if (item) {
                    entries.push({
                        id: item.id,
